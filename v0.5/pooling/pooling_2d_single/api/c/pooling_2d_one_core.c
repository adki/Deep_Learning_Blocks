...
int pool_set_addr( unsigned long int base )
{
    CSRA_POOL2D_BASE = base;

    CSRA_POOL2D_VERSION       =(base+0x00);
    CSRA_POOL2D_CONTROL       =(base+0x10);
    CSRA_POOL2D_CONFIG        =(base+0x14);
    CSRA_POOL2D_CONFIG_FIFO   =(base+0x18);
    
    CSRA_POOL2D_COMMAND       =(base+0x20);
    CSRA_POOL2D_KNL_CFG       =(base+0x24);
    
    CSRA_POOL2D_FTU_ADDR_LOW  =(base+0x30);
    CSRA_POOL2D_FTU_ADDR_HIGH =(base+0x34);
    CSRA_POOL2D_FTU_CFG_SIZE  =(base+0x38);
    CSRA_POOL2D_FTU_CFG_KNL   =(base+0x3C);
    CSRA_POOL2D_FTU_ITEMS     =(base+0x40);
    CSRA_POOL2D_FTU_BURST     =(base+0x44);
    CSRA_POOL2D_FTU_CHANNEL   =(base+0x48);
    
    CSRA_POOL2D_RST_ADDR_LOW  =(base+0x50);
    CSRA_POOL2D_RST_ADDR_HIGH =(base+0x54);
    CSRA_POOL2D_RST_CFG_SIZE  =(base+0x58);
    CSRA_POOL2D_RST_ITEMS     =(base+0x5C);
    CSRA_POOL2D_RST_BURST     =(base+0x60);
    
    CSRA_POOL2D_PROFILE_CTL   =(base+0x70);
    CSRA_POOL2D_PROFILE_CYCLES=(base+0x74);
    CSRA_POOL2D_PROFILE_CNT_RD=(base+0x78);
    CSRA_POOL2D_PROFILE_CNT_WR=(base+0x7C);
