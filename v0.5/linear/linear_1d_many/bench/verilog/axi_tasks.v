...
task read_task;
     ...
     input [31:0]         btype; // burst type
begin
     fork
     read_address_channel(id,addr,size,leng,btype);
     read_data_channel(id,addr,size,leng,btype);
     join
end
endtask
//----------------------------------------------------------------
...
begin
     fork
     write_address_channel(id,addr,size,leng,btype);
     write_data_channel(id,addr,size,leng,btype);
...
begin
     ...
     for (idx=0; idx<leng; idx=idx+1) begin
          ...
          WLAST <= #1 (idx==(leng-1));
          naddr <= get_next_addr(naddr, size, btype);
          @ (posedge ACLK);
...
function [WIDTH_AD-1:0] get_next_addr;
    ...
    input [31:0] size; // num of bytes in a beat
    input [31:0] btype; // type of burst
    integer offset;
begin
    case (btype[1:0])
    ...
    default: $display("%0t %m Error un-defined burst-type: %2b", $time, btype);
    endcase
end
endfunction
//----------------------------------------------------------------
