//------------------------------------------------------------------------------
    localparam P_NUM_MASTER  = 3; // should not be changed
    ...
    localparam P_WIDTH_DA    =`AXI_WIDTH_DA;// data width
...
//-----------------------------------------`NET_DELAY  ---------------------------------
     ...
     `endif
//------------------------------------------------------------------------------
    amba_axi_m3s2 #(.WIDTH_ID    (P_WIDTH_ID )
                   ,.WIDTH_AD    (P_WIDTH_AD )
                   ,.WIDTH_DA    (P_WIDTH_DA )
                   ,.WIDTH_DS    (P_WIDTH_DS )
                   ...
                   ,.SLAVE_EN1(1),.ADDR_BASE1(P_ADDR_BASE_LINEAR),.ADDR_LENGTH1($clog2(P_SIZE_LINEAR))
                   )
    u_bus (
                                        .ARESETn      ( ARESETn      )
     ,                                  .ACLK         ( ACLK         )
