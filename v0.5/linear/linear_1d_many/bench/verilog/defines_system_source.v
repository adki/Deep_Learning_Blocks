...
`define AMBA_AXI4
`define AMBA_AXI_WIDTH_AD    $AMBA_AXI_WIDTH_ADDR$ //32
`define AMBA_AXI_WIDTH_DA    $AMBA_AXI_WIDTH_DATA$ //32, 64, 128

`define DPU_DATA_TYPE        "$DATA_TYPE$" // "INTEGER", "FLOATING_POINT", "FIXED_POINT"
`define DPU_DATA_WIDTH       $DATA_WIDTH$ // 32, 16, 8
`ifdef  DPU_DATA_FIXED_POINT
`define DPU_DATA_WIDTH_Q     (`DPU_DATA_WIDTH/2) // fractional for fixed point
`endif

`define DPU_ADDR_BASE_MEM      'h00000000
...
`define DPU_SIZE_MEM           (16*1024*1024)
