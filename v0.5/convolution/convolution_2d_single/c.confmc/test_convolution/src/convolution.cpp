...
//------------------------------------------------------------------------------
int test_convolution_2d( uint32_t base, int test_level, int verbose )
{
    extern int test_convolution_2d_core(
                            ...
                            , int       rigor
                            , int       verbose );
    conv_set_addr((unsigned long int)base );
    #if defined(RIGOR)||defined(DEBUG)
    unsigned long int caddr = conv_get_addr();
    if (base!=(uint32_t)caddr) {
        PRINTF("convolution_2d at 0x%08lX, but 0x%08X expected\n", caddr, base);
    }
    #endif
    #if defined(DEBUG)
    conv_version();
