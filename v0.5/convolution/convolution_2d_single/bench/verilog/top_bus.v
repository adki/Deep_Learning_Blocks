//------------------------------------------------------------------------------
    localparam P_NUM_MASTER  = 4; // should not be changed
...
//-----------------------------------------`NET_DELAY  ---------------------------------
     ...
     `endif
//------------------------------------------------------------------------------
    amba_axi_m4s2 #(.WIDTH_ID    (P_WIDTH_ID )
                   ,.WIDTH_AD    (P_WIDTH_AD )
                   ,.WIDTH_DA    (P_WIDTH_DA )
                   ,.WIDTH_DS    (P_WIDTH_DS )
                   ...
                   ,.SLAVE_EN1(1),.ADDR_BASE1(P_ADDR_BASE_CONV),.ADDR_LENGTH1($clog2(P_SIZE_CONV))
                   )
    u_bus (
                                        .ARESETn      ( ARESETn      )
     ,                                  .ACLK         ( ACLK         )
