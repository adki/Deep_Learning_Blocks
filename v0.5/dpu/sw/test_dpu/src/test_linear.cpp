...
//------------------------------------------------------------------------------
int test_linear_1d( uint32_t base, int test_level, int verbose )
{
    extern int test_linear_1d_core( uint16_t  out_size
                                  ...
                                  , int       rigor
                                  , int       verbose );
    linear_set_addr((unsigned long int)base );
    #if defined(RIGOR)||defined(DEBUG)
    unsigned long int caddr = linear_get_addr();
    if (base!=(uint32_t)caddr) {
        PRINTF("linear_1d at 0x%08lX, but 0x%08X expected\n", caddr, base);
    }
    #endif
    #if defined(DEBUG)
    linear_version();
