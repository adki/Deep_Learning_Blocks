...
//------------------------------------------------------------------------------
int test_pooling_2d( uint32_t base, int test_level, int verbose )
{
    extern int test_pooling_2d_core( uint16_t  out_size    // only for square matrix
                                   ...
                                   , int       rigor
                                   , int       verbose );
    pool_set_addr((unsigned long int)base );
    #if defined(RIGOR)||defined(DEBUG)
    unsigned long int caddr = pool_get_addr();
    if (base!=(uint32_t)caddr) {
        PRINTF("pooling_2d at 0x%08lX, but 0x%08X expected\n", caddr, base);
    }
    #endif
    #if defined(DEBUG)
    pool_version();
