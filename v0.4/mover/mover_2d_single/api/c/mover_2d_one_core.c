...
int mover_set_addr( unsigned long int offset )
{
    CSRA_MOVER2D_BASE = offset;

    CSRA_MOVER2D_VERSION       =(offset+0x00);
    CSRA_MOVER2D_CONTROL       =(offset+0x10);
    CSRA_MOVER2D_CONFIG        =(offset+0x14);
    CSRA_MOVER2D_CONFIG_FIFO   =(offset+0x18);
    
    CSRA_MOVER2D_COMMAND       =(offset+0x20);
    
    CSRA_MOVER2D_SRCA_ADDR_LOW =(offset+0x30);
    CSRA_MOVER2D_SRCA_ADDR_HIGH=(offset+0x34);
    CSRA_MOVER2D_SRCA_CFG_SIZE =(offset+0x38);
    CSRA_MOVER2D_SRCA_ITEMS    =(offset+0x3C);
    CSRA_MOVER2D_SRCA_BURST    =(offset+0x40);
    
    CSRA_MOVER2D_SRCB_ADDR_LOW =(offset+0x50);
    CSRA_MOVER2D_SRCB_ADDR_HIGH=(offset+0x54);
    CSRA_MOVER2D_SRCB_CFG_SIZE =(offset+0x58);
    CSRA_MOVER2D_SRCB_ITEMS    =(offset+0x5C);
    CSRA_MOVER2D_SRCB_BURST    =(offset+0x60);
    
    CSRA_MOVER2D_RST_ADDR_LOW  =(offset+0x70);
    CSRA_MOVER2D_RST_ADDR_HIGH =(offset+0x74);
    CSRA_MOVER2D_RST_CFG_SIZE  =(offset+0x78);
    CSRA_MOVER2D_RST_ITEMS     =(offset+0x7C);
    CSRA_MOVER2D_RST_BURST     =(offset+0x80);
    
    CSRA_MOVER2D_FILL_VALUE    =(offset+0x90);
    CSRA_MOVER2D_ACTIV_FUNC    =(offset+0x94);
    CSRA_MOVER2D_ACTIV_PARAM   =(offset+0x98);
    
    CSRA_MOVER2D_PROFILE_CTL              =(offset+0xA0);
    CSRA_MOVER2D_PROFILE_RESIDUAL_OVERFLOW=(offset+0xA4);
    CSRA_MOVER2D_PROFILE_CYCLES           =(offset+0xA8);
    CSRA_MOVER2D_PROFILE_CNT_RD           =(offset+0xAC);
    CSRA_MOVER2D_PROFILE_CNT_WR           =(offset+0xB0);
